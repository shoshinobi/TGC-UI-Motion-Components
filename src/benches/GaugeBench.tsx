...
export function GaugeBench() {
  ...
  const [v, set] = useControls('Gauge', () => ({
    ...
    'pill flash': folder({
      ...
      flashBlend: { value: D.flashBlend, options: ['plus-lighter', 'screen', 'normal'], label: 'blend' },
      flashIntensity: { value: D.flashIntensity, min: 0, max: 4, step: 0.1, label: 'intensity (>1 stacks)' },
      flashBlur: { value: D.flashBlur, min: 0, max: 80, step: 1, label: 'blur (px)' },
      flashSpread: { value: D.flashSpread, min: 0, max: 40, step: 1, label: 'spread (px)' },
    }),
    appearance: folder(
      {
  ...
  const config: GaugeMotionConfig = {
    ...
    flashBlend: values.flashBlend as GaugeMotionConfig['flashBlend'],
